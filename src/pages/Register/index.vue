    ...
    <div class="register">
      ...
      <div class="controls">
        <input
          ...
          v-model="agreeTerms"
          name="agreeTerms"
          v-validate="{ required: true, agreeTerms: true }"
          :class="{ invalid: errors.has('agreeTerms') }"
        />
        <span>同意协议并注册《尚品汇用户协议》</span>
        <span class="error-msg">{{ errors.first("agreeTerms") }}</span>
      </div>
      <div class="btn">
...
export default {
  ...
  methods: {
    ...
    async userRegister() {
      ...
      if (success) {    // 全部表单验证成功, 再向服务器发请求, 只要有一个没有成功, 不发请求
        try {
          const { phone, code, password, comfirmPassword } = this;
          phone &&
            code &&
