...
export default {
  ...
  beforeMount() {
    ...
    // 在发请求之前,把接口需要传递的参数,尽心整理(在给服务器发送请求之前,把参数整理好, 服务器就会 返回查询的数据)
  ...
  methods: {
    ...
    attrInfo(attr, attrValue) {
      ...
      // this.searchParams.props = [...new Set(this.searchParams.props)]    //new Set 去重
      console.log();
      // 再次发请求
      this.getData();
    },
  ...
  // 数据监听: 监听组件身上的属性的属性值变化
  watch: {
    // 监听路由的信息是否发生变化, 如果发生变化, 再次发送请求
    $route(newValue, oldValue) {
      // 每一次请求完毕, 应该把相应的1, 2, 3级分类的id置空, 让它接收下一次响应的 1, 2, 3id
